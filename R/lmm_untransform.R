#' Untransform coefficient values back to the original scale for a *LMM* (no penalization)
#' 
#' This function unwinds both steps of the standardization process to obtain
#' coefficient values on their original scale. It is called by lmm_format().
#' 
#' @param res_b The values returned in the 'coefficient' argument of the lm.fit() object
...
lmm_untransform <- function(res_b, ns, ncol_X, std_X, SUX, std_SUX){
  ...
  untransformed_beta <- rep(0, ncol_X + 1) # + 1 is for the intercept
