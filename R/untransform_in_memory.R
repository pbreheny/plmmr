...
#' @keywords internal
#'
untransform_in_memory <- function(
  std_scale_beta,
  p,
  std_X_details,
  use_names = TRUE
  ...
  if (length(std_X_details$ns) == length(std_X_details$scale)) {
    ...
    untransformed_beta[1, ] <- a -
      crossprod(std_X_details$center, untransformed_b2)
  } else if (
    (length(std_X_details$ns) != length(std_X_details$scale)) &&
    ...
    untransformed_beta[std_X_details$ns + 1, ] <- untransformed_b2 # again, the + 1 is for the intercept
    untransformed_beta[1, ] <- a -
      crossprod(std_X_details$center[std_X_details$ns], untransformed_b2)
  } else {
