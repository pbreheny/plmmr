...
summary.plmm <- function(object, lambda, idx, eps = 1e-5, ...) {
  # lambda/which
  if (missing(lambda) && missing(idx)) {
    stop("One of the arguments 'lambda' or 'idx' must be provided.")
  }
  if (missing(lambda)) {
  ...
  if (missing(idx)) {
    ...
    if (sum(idx) == 0) {
      warning(
        "The user-specified lambda value is not within epsilon of any lambda values used to fit the model. Will proceed with the fitted model lambda value closest to the user-specified lambda."
      )
