...
#'
#' @param X Rotated and standardized design matrix which *includes* the intercept column if present. May include clinical covariates and other non-SNP data. This can be either a matrix or a filebacked `big.matrix` object.
...
#' @param lambda_min The smallest value for lambda, as a fraction of the maximum lambda. Default is .001 if the number of observations is larger than the number of covariates and .05 otherwise. A value of `lambda_min = 0` is not supported.
#' @param nlambda The desired number of lambda values in the sequence to be generated.
#' @param penalty_factor A multiplicative factor for the penalty applied to each coefficient. If supplied, `penalty_factor` must be a numeric vector of length equal to the number of columns of X. The purpose of `penalty_factor` is to apply differential penalization if some coefficients are thought to be more likely than others to be in the model.
#'                       In particular, `penalty_factor` can be 0, in which case the coefficient is always in the model without shrinkage.
#'
#' @return a numeric vector of lambda values, equally spaced on the log scale
#'
#' @keywords internal
#'
setup_lambda <- function(X, y, alpha, lambda_min, nlambda, penalty_factor) {
