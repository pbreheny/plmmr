...
-   $\mathbf{Z}$ is a $n \times b$ matrix of indicators corresponding to a grouping structure, and $\boldsymbol{\gamma}$ is the vector of values describing how each grouping is associated with $\mathbf{y}$. In real data, these values often unknown.
...
-   $\tilde{\dot{\mathbf{X}}} \equiv \mathbf{W}\mathbf{U}^\top\mathbf{X}$ is the rotated data, or the data on the transformed scale.

-   $\tilde{\mathbf{y}} \equiv \mathbf{\Sigma}^{-1/2}\mathbf{y}$ is the outcome on the rotated scale.

-   $\ddot{\tilde{\mathbf{X}}}$ is the **standardized** rotated data. Note: This standardization involves scaling, but not centering.

### Model fitting with penalization

We fit $\tilde{\mathbf{y}} \sim \ddot{\tilde{\mathbf{X}}} \hat{\ddot{\boldsymbol{\beta}}}$ using a penalized linear mixed model. The penalty parameter values (e.g., values of the lasso tuning parameter) are indexed by $\lambda_l \in 1,..., t$.
...
-   $\dot{\mathbf{X}}\dot{\boldsymbol{\beta}}$ is `Xb`
