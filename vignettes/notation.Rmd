...
-   Again, $\mathbf{K} \equiv \frac{1}{p}\dot{\mathbf{X}}\dot{\mathbf{X}}^{\top}$is often referred to in the literature as the realized relatedness matrix (RRM) or genomic relatedness matrix (GRM). $\mathbf{K}$ has dimension $n \times n$. Note that $\mathbf{K}$ can be obtained from the SVD of $\dot{\mathbf{X}}$, i.e., $\mathbf{U}\mathbf{S}\mathbf{U}^{\top}$, where $\mathbf{S}$ is the diagonal matrix whose diagonal values are $\frac{d_k^2}{p}$.
