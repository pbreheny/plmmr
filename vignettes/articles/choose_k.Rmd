...
Here we see that the starting value for $k$ is 19 -- by default, `choose_k()` makes the following 'decisions': 
  ...
  - The $\epsilon$ value used to determine when $||\mathbf{K} - \mathbf{A}_k||_{\text{F}}$ is satisfactorily small enough is automatically set to 2. This is controlled by the `eps` argument. 
  
If the user wants to have a smaller $\epsilon$ and start at a higher value, this is another option: 

```{r}
approx2 <- choose_k(X = admix$X, start = 30, eps = 1)
```
