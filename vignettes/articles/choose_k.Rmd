...
We can look at the $\mathbf{A}_k$ matrices produced by both approximations above, comparing to the true $\mathbf{K}$ (at far left):

```{r}
par(mfrow=c(1,3))
corrplot(approx1$K, is.corr = F)
corrplot(approx1$K_approx, is.corr = F)
corrplot(approx2$K_approx)
```
