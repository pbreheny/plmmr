...
```{r, include = FALSE}
set.seed(123) # Prevents parallel package warning about NULL seed when knitting
```

In this overview, we will provide a demo of the main functions in `plmmr` using the `admix` data. Check out the other vignettes to see examples of analyzing data from PLINK files or delimited files.
...
Notice: We are passing `admix$X` as the `design` argument in `plmm()`; internally, `plmm()` has taken this `X` input and created a `plmm_design` object. You could also supply `X` and `y` to `create_design()` to make this step explicit. 
...
Suppose we also know the ancestry groups with which each person in the `admix` data self-identified. We would probably want to include this in the model as an unpenalized covariate (i.e., we would want 'ancestry' to always be in the model). To specify an unpenalized covariate, we need to use the `create_design()` function prior to calling `plmm()`. Here is how that would look: 
...
```{r cvplot_par, fig.align='center', fig.width=8, fig.cap="Plot of CVE"}
