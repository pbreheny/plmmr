...
These data represent `r nrow(pen$X)` individuals and `r ncol(pen$X)` SNPs from a GWAS study. For the sake of example, let's use [hdl](https://www.cdc.gov/cholesterol/ldl_hdl.htm#:~:text=HDL%20(high%2Ddensity%20lipoprotein),for%20heart%20disease%20and%20stroke.) (high-density lipoprotein cholesterol) as our outcome. 
...
pen$y <- if_else(is.na(pen$fam$hdl), mean(pen$fam$hdl, na.rm = T), pen$fam$hdl)
...
pen$k_plus_pcs <- choose_k(X = pen$X_plus_fixed_eff, start = 1200, returnKapprox = T)

cv_pen3 <- cv.plmm(X = pen$X_plus_fixed_eff,
                   y = pen$y,
                   K = pen$k_plus_pcs$K_svd,
