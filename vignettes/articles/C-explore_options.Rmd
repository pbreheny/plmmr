...
It may be of interest to examine the bias and variance that result from cross-validated model fitting. `cv.plmm()` has the option `returnBiasDetails`; if set to `TRUE`, this will make the returned object include bias (a vector) and loss (a matrix). Moreover, for a `plmm` model fit, the function `construct_variance` will construct the approximated matrix $\mathbf{\hat V} \equiv \eta \mathbf{K} + (1 - \eta)\mathbf{I}_n$.
