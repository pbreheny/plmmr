...
```{r, eval=FALSE}
system(paste0("gunzip -k ",file.path(get_example_data(parent = TRUE),
                                     paste0("penncath_lite", "*"))))
```
...
```{r, eval=FALSE}
pen_cv_fit <- cv_plmm(X = std_X,
                      y = pen$fam$affection[pen$complete_phen],
                      std_needed = F, # std_X is already standardized
                      trace = T,
                      nfolds = 5)

print(summary(pen_cv_fit))
plot(pen_cv_fit)
plot(pen_cv_fit$fit)
```
...
fit_plus_newvars <- plmm(X = dat_plus_newvars,
                         ...
                         returnX = FALSE,
                         trace = TRUE)
# TODO: work through the example below 
