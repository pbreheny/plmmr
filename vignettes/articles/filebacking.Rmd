...
## Step 2: Create a design for analysis
...
fit <- plmm(design = pen_design, # will run in-memory by default, since data will fit
            returnX = TRUE, # must explicitly request the data to be read into memory
            trace = TRUE)
...
## Step 5: Cross-validation (tuning parameter selection)
...
Alternatively, we can pass the 'pen_design_rds' object to `cv_plmm()` for an equivalent analysis: 
...
```{r}
fb_mcp <- plmm(design = pen_design, penalty = "MCP", trace = TRUE)
...
And just as in the previous example, we can choose a value for $\lambda$ using cross validation: 

```{r}
cv_plus_newvars <- cv_plmm(design = new_pen_design,
                           type = 'blup',
                           penalty = "lasso",
                           nfolds = 3,
                           returnBiasDetails = TRUE,
...
plot(cv_plus_newvars)
summary(cv_plus_newvars)
```
