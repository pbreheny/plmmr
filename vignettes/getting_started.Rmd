...
`penalizedLMM` currently includes three example data sets: 
  ...
  * `cad_mid` is an upper mid-sized data set (1401 observations, 42117 SNPs) with several health outcomes as well as age and sex information. This data set is a larger subset the same data used as the reference for `cad_lite`.
...
cad_lite <- process_plink(prefix = "cad_lite",
                          dataDir = plink_example(path="cad_lite.fam", parent=T))
...
admix_pred1 <- predict(object = admix_fit, newX = admix$X, type = "response", idx=98)

# make prediction using blup 
admix_pred2 <- predict(object = admix_fit, newX = admix$X, type = "blup", idx=98)
