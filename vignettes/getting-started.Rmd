...
```{r}
admix_fit2 <- plmm(X = cbind(admix$race, admix$X), y = admix$y,
                   penalty = "lasso",
                   ...
                   penalty.factor = c(0, rep(1, ncol(admix$X))))
...
```{r admix_cv}
admix_cv <- cv.plmm(X = cbind(admix$race, admix$X), y = admix$y,
                    penalty = "lasso", penalty.factor = c(0, rep(1, ncol(admix$X))))
...
# our model
crossprod(admix$y - y_hat)/length(admix$y)
```
