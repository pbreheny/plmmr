---
title: "Working with PLINK files"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Working with PLINK files}
...
The `penncath_lite` data is in the PLINK format (i.e., bed/bim/fam files), so I need to do some preprocessing first. The `process_plink_2` function uses functions from the [bigsnpr package](https://privefl.github.io/bigsnpr/reference/index.html) to do some quality control filtering steps. The result of the `process_plink_2` function is an object with 1) a matrix of genotypes and 2) a vector with the indices of any constant features (e.g., SNPs with no variation across the sample). 

```{r penncath_lite_process, eval=FALSE}
# preprocess PLINK files, noting that these are gzipped
penncath_lite <- process_plink_2(
  data_dir = plink_example(path = "penncath_lite.bed.gz", parent = T),
  prefix = "penncath_lite",
  rds = F,
  gz = TRUE)

# NB: the 'quiet' option in process_plink_2()  will silence the printed messages
...
```{r penncath_lite_fit, eval=FALSE}
penncath_lite_fit <- plmm(X = penncath_lite$genotypes,
...
# penncath_lite_cv <- cv.plmm(X = penncath_lite$genotypes, 
#                        y = penncath_clinical$hdl_impute,
#                        K = relatedness_mat(penncath_lite$genotypes))
