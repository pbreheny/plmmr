...
cv1 <- cv_plmm(X = admix$X, y = admix$y, seed = 321, type = 'lp')
cv2 <- cv_plmm(X = admix$X, y = admix$y, seed = 321, type='blup')
if(identical(cv1$cve, cv2$cve))stop("BLUP and response types give same result")


# Test 2 (think of what to add here) -------------------------
# cv3 <- cv_plmm(X = admix$X, y = admix$y, k = 70, trace = TRUE)
# cv4 <- cv_plmm(X = admix$X, y = admix$y, k = 70, type = "blup", trace = TRUE)
