...
cv_args <- list(
  prep = prep,
  save_intermed = file.path(data_dir, "plmm_analysis_ofc/chr10_lite/cv_intermed_res.rds"),
  seed = 52242,
...
estimated_V <- construct_variance(eta = chr10_fit$eta, K = chr10_fit$K)
saveRDS(estimated_V, paste0(file.path(data_dir, "chr10_lite", "plmm_analysis_ofc"), '/estimated_V.rds'))
...
rds <- readRDS("/mnt/cleft/plmm_analysis_ofc/chr10_lite/cv_intermed_res.rds")
...
plot(cv_results)
summary(cv_results)
best_betas <- chr10_fit$beta_vals[,cv_results$min]
best_betas[abs(best_betas) > 1e-4]
coef(cv_results, lambda = cv_results$lambda_min)[abs(best_betas > 1e-4)]
