...
K_diagonal <- diag(x = (rnorm(n = nrow(admix$X))^2), nrow = nrow(admix$X))
...
lm1 <- lm(admix$y ~ admix$X, weights = 1 / v1)
...
expect_equivalent(
  matrix(0, nrow = length(monomorphic_snps), ncol = length(fit3$lambda)),
