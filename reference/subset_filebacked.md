...
- complete_samples:
  ...
  Numeric vector with indices marking the rows of the original data
  which have a non-missing entry in the 6th column of the `.fam` file
...
- columns are subset so that no constant features remain – this is
  important for standardization downstream

The list also includes the integer vector `ns` which marks which columns
of the original matrix were 'non-singular' (i.e. *not* constant
features). The `ns` index plays an important role in
